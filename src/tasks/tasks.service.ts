...
export class TasksService {
  ...
  getTaskById(id: string): Task {
    const found = this.tasks.find((task) => task.id === id); // find the task with the same id as the id we passed in
  ...
  deleteTask(id: string): void {
    // Error Handling
    const found = this.getTaskById(id); // Error Handling
    this.tasks = this.tasks.filter((task) => task.id !== found.id); // filter out the task with the same id as the id we passed in
  }
